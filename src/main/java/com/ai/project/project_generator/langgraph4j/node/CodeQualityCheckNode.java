...
import com.ai.project.project_generator.ai.CodeQualityCheckService;
...
public class CodeQualityCheckNode {
        ...
        return node_async(state -> {
            ...
            try {
                ...
                if (StrUtil.isBlank(codeContent)) {
                    ...
                    // 2. 调用 AI 进行代码质量检查
                    CodeQualityCheckService qualityCheckService = SpringContextUtil.getBean(
                        CodeQualityCheckService.class);
     ...
     * 需要检查的文件扩展名
     */
    private static final List<String> CODE_EXTENSIONS = Arrays.asList(".html", ".htm", ".css", ".js", ".json", ".vue",
        ".ts", ".jsx", ".tsx");
    ...
    private static boolean shouldSkipFile(File file, File rootDir) {
        ...
        // 跳过特定目录下的文件
        return relativePath.contains("node_modules" + File.separator) || relativePath.contains("dist" + File.separator)
            || relativePath.contains("target" + File.separator) || relativePath.contains(".git" + File.separator);
    }
